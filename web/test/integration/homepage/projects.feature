Feature:  Display public projects on the homepage
  ...
  Scenario: List public project on the homepage with minimum information
    Given there is a document "test-public-project" with the json "test/data/projects/project-public-minimum.json" in collection "projects"
    And there is a document "test-user-minimum" with the json "test/data/users/user-minimum.json" in collection "users"
    And there is a document "test-user-minimum" with the json "test/data/user-stats/user-minimum.json" in collection "userStats"
  ...
  Scenario: List public project on the homepage with repository and monitor
    Given there is a document "test-public-project" with the json "test/data/projects/project-public-full.json" in collection "projects"
    Given there is a document "test-repository-minimum" with the json "test/data/repositories/repository-minimum.json" in collection "repositories"
    And there is a document "test-user-minimum" with the json "test/data/users/user-minimum.json" in collection "users"
    And there is a document "test-user-minimum" with the json "test/data/user-stats/user-minimum.json" in collection "userStats"
    When the "/" page is open
