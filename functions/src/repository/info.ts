...
import { GitHubIssuesInput, GitHubIssuesMapper } from '../mappers/github/issues.mapper';
...
export const getRepositoryInfo: any = async (token: string, fullName: string) => {
  const data: [GitHubRepositoryInput, GitHubPullRequestInput[], GitHubEventInput[], GitHubReleaseInput[], GitHubIssuesInput[]] = await Promise.all([
    ...
    GitHubClient<GitHubIssuesInput[]>(`/repos/${fullName}/issues`, token),
  ]);
  const mappedData: GitHubRepositoryModel = {
    ...
    issues: data[4].map((issues: GitHubIssuesInput) => GitHubIssuesMapper.import(issues)),
  }
